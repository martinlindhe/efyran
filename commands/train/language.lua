...
return function()
    ...
    for i = 1, maxLanguageID do
        local currentSkill = mq.TLO.Me.LanguageSkill(i)()
        if currentSkill > 0 then
            local attempts = maxAttempts - currentSkill

            all_tellf("Training language %s (id %d, skill %d) for %d attempts ...", mq.TLO.Me.Language(i)(), i, currentSkill, attempts)
            cmdf("/language %d", i)
            delay(10)
