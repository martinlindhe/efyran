...
local function getMezzSpell(category)
    ...
    for _, mezzSpell in ipairs(spellGroup) do
        if is_memorized(mezzSpell.Name) then
            return mezzSpell
        end
    end
...
local function canCast(spell)
    ...
    if not is_spell_ready(spell.Name()) then
        return false
    end
...
local function doSingleMezz()
    ...
    local mqSpell = get_spell(mezzSpell.Name)
    if mqSpell == nil then
        all_tellf("UNILKELY: doSingleMezz failed to get_spell %s", mezzSpell.Name)
        return
    end
    ...
    for i=1, mezzTargetCount do
        ...
        elseif maTargetId ~= mezzSpawn.ID() and IsMaybeAggressive(mezzSpawn) and canCastOn(mezzSpawn, mqSpell) then
